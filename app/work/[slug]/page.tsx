...
export default async function ProjectPage({
  ...
  return (
    <main className="relative min-h-svh bg-white text-foreground">
      ...
      {hasCaseStudyHeader ? (
        // ── Case-study header: light info bar + full-width hero image ─────
        <>
          <div className="bg-white pt-16 md:pt-20">
            ...
            <div className="mx-auto max-w-5xl px-4 pb-6 md:px-8">
              <Link
                href="/#work"
                className="group inline-flex items-center gap-2 font-serif text-sm font-light lowercase text-foreground/50 transition-colors hover:text-foreground"
              >
            ...
            <div className="mx-auto max-w-5xl px-4 pb-12 md:px-8 md:pb-16">
              ...
              <div className="flex flex-col items-center gap-8 text-center md:hidden">
                <div>
                  <h1 className="font-light text-4xl text-foreground">{project.title}</h1>
                  {project.subtitle && (
                    <p className="mt-1 font-serif font-light text-foreground/50">
                      {project.subtitle}
                    </p>
                  )}
                </div>
                <div>
                  <p className="mb-3 font-serif text-xs font-medium uppercase tracking-[0.15em] text-foreground/40">
                    Scope
                  </p>
                  <div className="flex flex-col items-center gap-1.5">
                    {project.scope!.map((s) => (
                      <span
                        key={s}
                        className="font-serif text-sm text-foreground/70"
                      >
                        {s}
                      </span>
                    ))}
                ...
                <div>
                  <p className="mb-2 font-serif text-xs font-medium uppercase tracking-[0.15em] text-foreground/40">
                    About
                  </p>
                  {project.year && (
                  ...
                  {project.about && (
                    <p className="mt-2 font-serif font-light text-sm leading-relaxed text-foreground/70 max-w-xs mx-auto">
                      {project.about}
                    </p>
                  )}
                </div>
              ...
              <div className="hidden md:flex md:items-start md:gap-12">
                ...
                <div className="flex-1 min-w-0">
                  <h1 className="font-light text-5xl leading-tight text-foreground">
                    {project.title}
                  </h1>
                  {project.subtitle && (
                    <p className="mt-2 font-serif font-light text-foreground/50 text-lg">
                      {project.subtitle}
                    </p>
                  )}
                </div>
                ...
                <div className="w-52 shrink-0">
                  <p className="mb-4 font-serif text-xs font-medium uppercase tracking-[0.15em] text-foreground/40">
                    Scope
                  </p>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {project.year && (
                    <p className="font-serif text-sm text-foreground/50 mb-2">
                      {project.year}
                    </p>
                  )}
                  {project.about && (
                    <p className="font-serif font-light text-sm leading-relaxed text-foreground/70">
                      {project.about}
                    </p>
                  )}
                </div>
